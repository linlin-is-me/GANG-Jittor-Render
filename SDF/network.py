...
class SDF(nn.Module):
    ...
    def execute(self, points, iter_nr=20000):
    ...
    def get_sdf_and_gradient(self, points, iter_nr=20000, method="autograd"):
        ...
        if method=="finite_difference":
            with jt.set_grad_enabled(False):
                ...
                points_full=jt.concat([points, points_xplus, points_yplus, points_zplus],0)

               
            sdf_full, geom_feat_full = self.execute(points_full, iter_nr)
            ...
            gradients=jt.concat([grad_x, grad_y, grad_z],1)


        elif method=="autograd":

            #do it with autograd
            with jt.set_grad_enabled(True):
                points.requires_grad = True
                sdf, geom_feat = self.execute(points, iter_nr)

                feature_vectors=None
                gradients = jt.grad(sdf.sum(), points)
...
def map_range_val( input_val, input_start, input_end,  output_start,  output_end):
    # input_clamped=jt.clamp(input_val, input_start, input_end)
