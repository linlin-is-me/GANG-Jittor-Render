...
def get_ratio(output_dir,gt_dir):
    ...
    lpips_fn = LPIPS(net="vgg") if LPIPS is not None else None
    normal_gt_stack = []
    normal_gs_stack = []
    ...
    for test_dir in tqdm(test_dirs):
        ...
        albedo_gt = jt.array(albedo_gt) / 255.0  # [H, W, 3]
        albedo_gts.append(albedo_gt)
        mask = jt.array(mask)  # [H, W]
        masks.append(mask)
        ...
        albedo_map = jt.array(albedo_map) / 255.0  # [H, W, 3]
        albedo_maps.append(albedo_map)
        reconstructed_albedo_list.append(albedo_map[mask])
    gt_albedo_all = jt.concat(gt_albedo_list, dim=0)
    albedo_map_all = jt.concat(reconstructed_albedo_list, dim=0)
